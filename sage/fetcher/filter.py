...
def _names_match(cpe_name: str, pkg_name: str) -> bool:
    ...
    ALIASES: dict[str, set[str]] = {
        ...
        "express":        {"express", "expressjs_express", "express_express"},
        "axios":          {"axios", "axios_axios", "matteocollina_axios"},
        "lodash":         {"lodash", "lodash_lodash", "jashkenas_lodash"},
        "moment":         {"moment", "moment_moment", "momentjs"},
        "webpack":        {"webpack", "webpack_webpack", "webpack_js"},
        "react":          {"react", "facebook_react", "reactjs"},
        "next":           {"next", "vercel_next_js", "nextjs", "next_js"},
        "vue":            {"vue", "vuejs_vue", "vue_vue"},
        "angular":        {"angular", "angular_angular"},
        "jquery":         {"jquery", "jquery_jquery"},
        "typescript":     {"typescript", "microsoft_typescript"},
        "eslint":         {"eslint", "eslint_eslint"},
        "jest":           {"jest", "facebook_jest", "jestjs"},
        "babel":          {"babel", "babel_babel", "babeljs"},
        "node_fetch":     {"node_fetch", "node-fetch", "node_fetch_node_fetch"},
        "got":            {"got", "sindresorhus_got"},
        "superagent":     {"superagent", "visionmedia_superagent"},
        "jsonwebtoken":   {"jsonwebtoken", "auth0_node_jsonwebtoken", "jwt"},
        "passport":       {"passport", "jaredhanson_passport"},
        "bcrypt":         {"bcrypt", "bcryptjs", "node_bcrypt"},
        "dotenv":         {"dotenv", "motdotla_dotenv", "dotenv_dotenv"},
        "socket_io":      {"socket_io", "socket.io", "socketio", "socketio_socket_io"},
        "multer":         {"multer", "expressjs_multer"},
        "cors":           {"cors", "expressjs_cors"},
        "helmet":         {"helmet", "helmetjs_helmet"},
        "morgan":         {"morgan", "expressjs_morgan"},
        "mongoose":       {"mongoose", "automattic_mongoose"},
        "sequelize":      {"sequelize", "sequelize_sequelize"},
        "pg":             {"pg", "brianc_node_postgres", "node_postgres"},
        "mysql2":         {"mysql2", "sidorares_node_mysql2"},
        "redis":          {"redis", "redis_node_redis"},
        "uuid":           {"uuid", "uuidjs_uuid"},
        "sharp":          {"sharp", "lovell_sharp"},
        "tar":            {"tar", "npm_node_tar", "node_tar"},
        "semver":         {"semver", "npm_node_semver"},
        "minimist":       {"minimist", "minimist_minimist"},
        "ansi_regex":     {"ansi_regex", "ansi-regex", "chalk_ansi_regex"},
        "glob":           {"glob", "isaacs_node_glob"},
        "rimraf":         {"rimraf", "isaacs_rimraf"},
        "cross_spawn":    {"cross_spawn", "cross-spawn"},
        "debug":          {"debug", "debug_js_debug"},
    }
